...
ComparaMujeres <- function(periodo.enoe,
   ...
   source("01_scripts\\03_CalcularXNiveles_M.R")
   source("01_scripts\\04_CalcularXSTEM_M.R")
   source("01_scripts\\05_CalcularXCarreras_M.R")
   source("01_scripts\\07_CalcularXAreas_M.R")
   source("01_scripts\\08_CalcularXSubAreas_M.R")
   ...
   Calculos <- function(periodo.enoe, fac_inf, version.cmpe){
      ...
      resultados[[1]] <- CalcularXNiveles_M(enoesvy, periodo.enoe)
      
      resultados[[2]] <- CalcularXAreas_M(enoesvy, clas.car, periodo.enoe)
      
      resultados[[3]] <- CalcularXSubAreas_M(enoesvy, clas.car, periodo.enoe)
      
      resultados[[4]] <- CalcularXSTEM(enoesvy, periodo.enoe)
      
      resultados[[5]] <- CalcularXCarreras_M(enoesvy, clas.car, periodo.enoe)
      
      names(resultados) <- c("niveles", "areas", "subareas", "stem", "carreras")
   ...
   if(length(periodo.enoe) == 1){
      ...
      if(length(periodo.enoe) > 1){
         ...
         res_juntos <- list(data.frame(), data.frame(), data.frame(), data.frame(), data.frame())
         ...
         for(i in 1:length(res_juntos)){
            
            periodos <- as.character(unique(res_juntos[[i]][, "PERIODO"]))
            periodos <- paste(periodos, collapse = "/")
            
            res_juntos[[i]] <- res_juntos[[i]] %>% 
               group_by_at(c(1:5)) %>% 
   ...
   if(guardar == "Si"){
      ...
      periodo <- paste(periodo.enoe, collapse = "-")
      
      saveRDS(resultados, paste("03_resultados\\temporales\\resultados", periodo, ".RDS", sep= "", collapse = ""))
      
      cat("resultados guardados para periodo", periodo, "\n")
